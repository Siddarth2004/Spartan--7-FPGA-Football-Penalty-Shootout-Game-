...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE write_data[2] = {reg+2,val};
	int status = XSpi_Transfer(&SpiInstance, write_data,NULL,2);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int status = XSpi_Transfer(&SpiInstance, write_data,NULL,nbytes+1);
...
//      //return val
	...
	BYTE command_data[1] = {reg};

	BYTE read_data[2];

	int status = XSpi_Transfer(&SpiInstance, command_data,read_data,2);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int status = XSpi_Transfer(&SpiInstance, command_data,read_data,nbytes+1);

	for(int i = 1; i <= nbytes; i++){

		data[i-1] = read_data[i];
	}
